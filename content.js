...
function removeGames() {
  ...
  const conditions = ['league of legends',`playerunknown's battlegrounds`, 'counter-strike: global offensive', 'pubg mobile', 'grand theft auto v',
  ...
  for (let i = 0; i < cards.length; i++) {
    ...
    try {
      ...
      if(cards[i].parentElement.parentElement.parentElement.parentElement.parentElement.hasChildNodes){
        ...
        if(conditions.some(el => cards[i].children[1].children[1].children[0].children[1].children[1].children[0].children[0].textContent.toLowerCase().includes(el))){
